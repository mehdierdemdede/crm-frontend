...
interface FetchJsonOptions<TRequest, TResponse> {
    method?: string;
    body?: TRequest;
...
async function fetchJson<TRequest, TResponse>(
    path: string,
    { method = "GET", body, headers, requestSchema, responseSchema }: FetchJsonOptions<TRequest, TResponse>,
): Promise<TResponse> {
    if (!BILLING_BASE_URL) {
    ...
    const response = await fetch(`${BILLING_BASE_URL}${path}`, requestInit);
    ...
    if (!parsedResponse.success) {
        throw new ApiError("Invalid response payload", response.status, parsedResponse.error.flatten());
    }
...
export const createSubscription = async (
    payload: z.infer<typeof ZCreateSubscription>,
): Promise<Subscription> =>
    fetchJson<typeof payload, Subscription>("/billing/subscriptions", {
        method: "POST",
        body: payload,
        requestSchema: ZCreateSubscription,
        responseSchema: ZSubscription,
    });

export const changeSubscriptionPlan = async (
    subscriptionId: string,
    payload: z.infer<typeof ZChangePlan>,
): Promise<Subscription> =>
    fetchJson<typeof payload, Subscription>(`/billing/subscriptions/${subscriptionId}/change-plan`, {
        method: "POST",
        body: payload,
        requestSchema: ZChangePlan,
        responseSchema: ZSubscription,
    });

export const updateSubscriptionSeats = async (
    subscriptionId: string,
    payload: z.infer<typeof ZUpdateSeats>,
): Promise<Subscription> =>
    fetchJson<typeof payload, Subscription>(`/billing/subscriptions/${subscriptionId}/seats`, {
        method: "POST",
        body: payload,
        requestSchema: ZUpdateSeats,
        responseSchema: ZSubscription,
    });

export const cancelSubscription = async (
    subscriptionId: string,
    payload: z.infer<typeof ZCancelSub>,
): Promise<Subscription> =>
    fetchJson<typeof payload, Subscription>(`/billing/subscriptions/${subscriptionId}/cancel`, {
        method: "POST",
        body: payload,
...
export const getCustomerSubscriptions = async (customerId: string): Promise<Subscription[]> =>
    fetchJson<void, Subscription[]>(`/billing/customers/${customerId}/subscriptions`, {
        responseSchema: ZSubscriptionsResponse,
    });
